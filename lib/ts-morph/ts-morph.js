...
import { errors, Memoize, KeyValueCache, InMemoryFileSystemHost as RealFileSystemHost, TransactionalFileSystem, CompilerOptionsContainer, ObjectUtils, WeakCache, EventContainer, DocumentRegistry, createHosts, StringUtils, FileUtils, SortedKeyValueArray, LocaleStringComparer, ArrayUtils } from "./ts-morph-common";
import { CodeBlockWriter } from "../code-block-writer/code-block-writer";
...
function printNode(node, sourceFileOrOptions, secondOverloadOptions) {
    ...
    const printer = ts.createPrinter({
        newLine: (_a = options.newLineKind) !== null && _a !== void 0 ? _a : ts.NewLineKind.LineFeed,
...
class NamespaceDeclaration extends NamespaceDeclarationBase {
    ...
    getDeclarationKind() {
        ...
        return declarationKeyword.getKind() === ts.SyntaxKind.NamespaceKeyword ? NamespaceDeclarationKind.Namespace : NamespaceDeclarationKind.Module;
    }
...
class Symbol {
    ...
    getExport(name) {
        if (this.compilerSymbol.exports == null)
            return undefined;
        const tsSymbol = this.compilerSymbol.get(ts.escapeLeadingUnderscores(name));
        return tsSymbol == null ? undefined : this._context.compilerFactory.getSymbol(tsSymbol);
    }
    getExports() {
        if (this.compilerSymbol.exports == null)
...
function createWrappedNode(node, opts = {}) {
    ...
    const projectContext = new ProjectContext(undefined, new TransactionalFileSystem(new RealFileSystemHost()), compilerOptionsContainer, { createLanguageService: false, typeChecker });
